...
version = "fever_2017_9_22_4way_os"
session = create_session()

result = session.execute(
    """
    select sentence.text as st, original.text as ot, mutation.text as mt, mutation.mutation_type_id, sentence.entity_id, original.id as clid
...
done_original_claims = set()
for claim in tqdm(result):
    if claim['mutation_type_id'] not in ["negate","rephrase","general","specific","gen","spec","neg"]:
        continue

    st_tokens, st_pos = tok_pos(claim['st'])
    ot_tokens, ot_pos = tok_pos(claim['ot'])
    mt_tokens, mt_pos = tok_pos(claim['mt'])

    if claim['clid'] not in done_original_claims:
        claims[claim["entity_id"]].append({
            "original":" ".join(st_tokens),
            "original_pos": " ".join(st_pos),
            "mutation":" ".join(ot_tokens),
            "mutation_pos": " ".join(ot_pos),
            "type":"original"})
        done_original_claims.add(claim['clid'])
